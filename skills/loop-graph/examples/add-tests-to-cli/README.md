...
The generated `executor.md` and `ledger.md` are in this folder (in a real run they'd land in `.longgraph/<date-slug>/` at the repo root — they sit here for readability). The ledger shows three rounds already run — including a **register-then-defer** in Round 2 (a timezone bug found while writing a test, logged as `GAP-002` instead of fixed on the spot) and that gap being picked up in Round 3.
