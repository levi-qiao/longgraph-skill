...
Hosts also differ in **what a round starts from**, which is the main driver of what a run costs: most resume the previous round's context rather than booting cold (Grok chains each fire in a detached subagent and then chops the chain wherever it runs out; Codex appends to one thread; Cursor and Claude Code stay in-session; only shell/cron is fresh per tick). So finer rounds are *not* cheaper — each carries every earlier round's output again — and a supervisor tick is only clean if the host is made to start clean. `/octopus` sizes rounds and plans the context resets for the host you name; [`host-dialects.md`](../../lib/host-dialects.md) holds the per-host mechanics.
