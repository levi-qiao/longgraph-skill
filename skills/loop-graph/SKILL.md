...
For each supervised milestone boundary, name its exact promotion audit surface and exit checks. Seed Gate-wait work only when it is clearly independent, one-round, verified, and write-disjoint; otherwise omit it. Fix the **owner-only decision list** (typical: DDL/schema, credentials / remote env / real-data exposure, spend beyond budget, lowering a metric bar, frozen contracts, push) — everything off that list the supervisor decides itself.
