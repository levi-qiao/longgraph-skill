...
Set two cadences and one batch size. `EXEC_INTERVAL` is roughly one fire's work — several rounds plus their gates. `SUP_INTERVAL` is 3–4× that, phase-offset. `ROUNDS_PER_FIRE` (default 3) is how many verified rounds one fire closes: a round is the smallest independently verifiable increment, but every fire boundary pays for re-derivation, so batch siblings that share one verification and let the fire close several rounds on warm context rather than paying a cold start per round.
...
- Tune the knobs; otherwise use `ROUNDS_PER_FIRE=3`, `CONVERGE_EVERY=5`, `NET_LINE_CAP=400`, `KEEP_ROUNDS=5`, `OPEN_DIRECTIVE_CAP=8`, `STANDING_CAP=12`, `GAP_CAP=12`, and `FILE_LINE_CAP=200`.
