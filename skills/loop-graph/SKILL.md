...
- **Stop conditions end the loop.** An **intermediate** milestone going all-green does **not** stop the run when a supervisor is present: the executor sets the ledger's **Milestone gate to `pending-audit`**, records the exact promotion audit surface, and keeps looping while the supervisor independently re-verifies that boundary. During the wait it may take one preplanned **Gate-wait backlog** item per round, but only from the generated ledger: each item is dependency-free, one-round, narrowly verified, and has an exact write set disjoint from the boundary, shared/global surfaces, the next milestone, and every peer item. Runtime debt never enters this lane; convergence waits until release. The supervisor audits promotion and completed Gate-wait rows separately, so a backlog failure cannot block a valid promotion unless it contaminated the boundary. The executor starts the next milestone only after the milestone acceptance directive lands; otherwise it cheap-ticks, and the stall guard ignores this wait state. Human sign-off is reserved for the final / North Star promotion, an owner-only boundary, or a run with no supervisor. All remaining items blocked → stop, escalate. Two rounds with no ledger/metric change (**except while parked at a `pending-audit` gate**) → stop, stall diagnosis. Red line violated → stop immediately. Terminal states end both loops.
